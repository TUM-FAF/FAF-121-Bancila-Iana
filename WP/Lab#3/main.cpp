...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    int xFillPreview   = 650;
    int yFillPreview   = 61;
    int xStrokePreview = 650;
    int yStrokePreview = 32;


    HDC hdcMem;
    BITMAP bitmap;
    HBITMAP hbmpFloraImage = NULL;
    hbmpFloraImage = (HBITMAP)LoadImage(hInst, "flora.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    GetObject(hbmpFloraImage, sizeof(bitmap), &bitmap);

    static RECT drawingArea = {14, 100, 780, 475};
    ...
    static RECT gradientRect = {710, 25, 770, 85};
    ...
    switch(msg)
    {
        ...
        case WM_PAINT:
            ...
            SelectObject(hdcMem, hbmpFloraImage);
            BitBlt(hdc, 15, 1, bitmap.bmWidth, bitmap.bmHeight, hdcMem, 0, 0, SRCCOPY);
            DeleteDC(hdcMem);
            ...
            for(int i = 0; i < (gradientRect.right - gradientRect.left); i++) {
                int blue;
                blue = i * 255 / (gradientRect.right - gradientRect.left);
        ...
        case WM_CLOSE:
            if(MessageBox(hwnd, "Are you sure that you want to close this task tracker?", "Lab#3", MB_YESNO) == IDYES)
                DestroyWindow(hwnd);
            break;
